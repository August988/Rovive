...
<div class="content">
    ...
    <div id="CatalogFrame" name="CatalogFrame">
        <?php
...
<script defer>
    let catalogCategory = 0;
    let catalogType = 0;


    $("#categoriesForKeyword").change(function() {
        catalogCategory = $(this).val();
        refreshCatalog();
    });
    ...
    function refreshCatalog() {
        ...
        $.ajax({
            ...
            success: function(data) {
                let dataReceived = data.Items;
                let html = "";

                for (let i = 0; i < dataReceived.length; i++) {
                    let item = dataReceived[i];

                    html += `
                                <div class="CatalogItem">
                                    <div class="CatalogItemInside">
                                        <div class="CatalogItemInsideInner">
                                            <div class="roblox-item-image image-small" data-item-id="${item.AssetId}" data-image-size="small">
                                                <div class="item-image-wrapper"><a href="/catalog/${item.AssetId}/item"> <img title="${item.Name}" alt="${item.Name}" class="original-image" src="${item.Thumbnail.Url}"> <img src="https://web.archive.org/web/20160429030338im_/https://images.rbxcdn.com/d649b9c54a08dcfa76131d123e7d8acc.png" alt="Limited Unique" class="limited-overlay"> <img src="https://web.archive.org/web/20160429030338im_/https://images.rbxcdn.com/b84cdb8c0e7c6cbe58e91397f91b8be8.png" alt="New"> </a></div>
                                            </div>
                                            <div class="CatalogItemInfo">
                                                <div class="CatalogItemName notranslate"><a class="name notranslate" href="/catalog/${item.id}/item" title="${item.Name}">${item.Name}</a></div>
                                                <div class="robux-price"><span class="SalesText"></span><span class="robux notranslate">${item.Price}</span></div>
                                            </div>
                                            <div class="CatalogHoverContent">
                                                <div><span class="CatalogItemInfoLabel">Creator:</span> <span class="HoverInfo notranslate"><a href="/users/${item.Creator}/profile">Not implemented</a></span></div>
                                                <div><span class="CatalogItemInfoLabel">Updated:</span> <span class="HoverInfo">Not implemented</span></div>
                                                <div><span class="CatalogItemInfoLabel">Sales:</span> <span class="HoverInfo notranslate">Not implemented</span></div>
                                                <div><span class="CatalogItemInfoLabel">Favorited:</span> <span class="HoverInfo">Not implemented</span></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
